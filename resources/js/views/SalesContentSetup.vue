        ...
        <ICardBody>
          ...
          <IFormGroup label-for="call_script">
            <IFormTextarea
              ...
              rows="6"
              placeholder="Hi, my name is [name] from [company]…"
            />
            <IFormText
          ...
          <IFormGroup label="Client Reviews" label-for="client_review_link">
            <IFormInput
              ...
              type="url"
              placeholder="https://g.page/… or https://testimonials…"
            />
          </IFormGroup>
          ...
          <IFormGroup label="Intro Message — shown at 'Contact Made' step" label-for="tpl_intro">
            <IFormTextarea
              id="tpl_intro"
              v-model="templateMap.intro"
              rows="4"
              placeholder="Hi {name}, this is {salesman_name} from {company_name}…"
            />
          </IFormGroup>

          <IFormGroup label="Sample Message — shown at 'Sample Shared' step" label-for="tpl_sample">
            <IFormTextarea
              id="tpl_sample"
              v-model="templateMap.sample"
              rows="4"
              placeholder="Hi {name}, please check our portfolio…"
            />
          </IFormGroup>

          <IFormGroup label="Estimate Message — shown at 'Estimate Shared' step" label-for="tpl_estimate">
            <IFormTextarea
              id="tpl_estimate"
              v-model="templateMap.estimate"
              rows="4"
              placeholder="Hi {name}, here is the estimate for your event…"
            />
          </IFormGroup>

          <IFormGroup label="Follow-up Message — shown at 'Negotiation' &amp; 'Follow-up' steps" label-for="tpl_followup">
            <IFormTextarea
              id="tpl_followup"
              v-model="templateMap.followup"
              rows="4"
              placeholder="Hi {name}, just following up on our conversation…"
            />
          </IFormGroup>
...
async function fetchContent() {
  loading.value = true
  try {
    const { data } = await Innoclapps.request().get('/akash-sales-pipeline/sales-content')
...
async function save() {
  saving.value = true
  try {
    await Innoclapps.request().post('/akash-sales-pipeline/sales-content', {
