    ...
    <div>
      <ICardHeader>
        <div>
          ...
          <IText
            class="block"
            :text="'Showing all pending follow-ups for today â€” ' + todayLabel"
          />
        </div>
...
async function fetchFollowups() {
  loading.value = true
  try {
    const { data } = await Innoclapps.request().get('/acp-sales-guide/followups/today')
...
async function markDone(f) {
  try {
    await Innoclapps.request().put(`/acp-sales-guide/followups/${f.id}/today-done`)
